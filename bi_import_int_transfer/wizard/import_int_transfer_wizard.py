...
class ImportIntTransferWizard(models.TransientModel):
    ...
    location_id = fields.Many2one(
        ...
        "Source Location",
        required=True,
        domain="[('id', '!=',  location_dest_id), ('usage','=','internal'),"
    ...
    location_dest_id = fields.Many2one(
        ...
        required=True,
        domain="[('id', '!=',  location_id),('usage','=','internal'),"
        " '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
    )
    ...
    import_type = fields.Selection(
        [("csv", "CSV File"), ("excel", "Excel File")], default="csv", string="Import File Type", required=True
    )
        ...
        if self and self.file and self.location_id and self.location_dest_id and self.scheduled_date:
                ...
                try:
                    ...
                    if search_warehouse:
                        search_picking_type = self.env["stock.picking.type"].search(
                            [("code", "=", "internal"), ("warehouse_id", "=", search_warehouse.id)], limit=1
                        )
                            ...
                            if row[0] not in (None, ""):
                                ...
                                if search_product and search_product.type in ["product", "consu"]:
                                    ...
                                    if row[2].strip() not in (None, ""):
                                        ...
                                        skipped_line_no[
                                            str(counter)
                                        ] = " - Unit of Measure not defined for this product. "
                                        counter = counter + 1
                                        continue
                                    ...
                                    skipped_line_no[
                                        str(counter)
                                    ] = " - Product not found or it's not a Stockable or Consumable Product. "
                                    counter = counter + 1
                                    continue
                            else:
                    ...
                    raise UserError(_("Please choose the csv file for import or change the import file type."))
            ...
            if self.import_type == "excel":
                ...
                try:
                            ...
                            if sheet.cell(row, 0).value not in (None, ""):
                                ...
                                if search_product and search_product.type in ["product", "consu"]:
                                    ...
                                    if sheet.cell(row, 1).value not in (None, ""):
                                        vals.update(
                                            {
                                                "product_uom_qty": sheet.cell(row, 1).value,
                                            }
                                        )
                                    else:
                                        vals.update({"product_uom_qty": 0.0})
                    ...
                    raise UserError(_("Please choose the excel file for import or change the import file type"))
