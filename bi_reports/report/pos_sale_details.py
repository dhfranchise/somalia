...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
        ...
        # Added seperate loop for group by products
        for line in orders.mapped("lines").read_group(
            domain=[], fields=["product_id", "price_unit", "discount", "qty"], groupby=["product_id"]
        ):
            product_id = self.env["product.product"].browse(line["product_id"][0])
            key = (product_id, line["price_unit"], line["discount"])
