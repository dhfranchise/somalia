...
class ImportPOLWizard(models.TransientModel):
    ...
    def validate_field_selection(self, field_name, field_ttype, field_value, field_required, field_name_m2o):
        ...
        if selection_key_value_list and field_value not in (None, ""):
            ...
            return {
                "error": " - " + field_name + " given value " + str(field_value) + " does not match for selection. "
            }
    ...
    def import_sol_apply(self):
        sol_obj = self.env["purchase.order.line"]
        ir_model_fields_obj = self.env["ir.model.fields"]
        # perform import lead
        if self and self.file and self.env.context.get("bi_po_id", False):
            # For CSV
                            ...
                            if row[0] != "":
                                ...
                                if search_product:
                                    ...
                                    if row[5].strip() in (None, "") and search_product.taxes_id:
                                        vals.update({"taxes_id": [(6, 0, search_product.taxes_id.ids)]})
                                    else:
                                        taxes_list = []
                                ...
                                vals.update({"order_id": self.env.context.get("bi_po_id")})
