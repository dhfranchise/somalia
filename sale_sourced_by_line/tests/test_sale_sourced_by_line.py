...
class TestSaleSourcedByLine(TransactionCase):
    def setUp(self):
        ...
        self.warehouse1 = self.stock_warehouse_model.create({"name": "Test Warehouse", "code": "TWH"})
    ...
    def test_sales_order_multi_source(self):
        ...
        for line in so.order_line:
            ...
            moves = self.stock_move_model.search([("group_id", "=", line.procurement_group_id.id)])
            for move in moves:
                self.assertEqual(
                    move.group_id,
    ...
    def test_sales_order_no_source(self):
        ...
        self.sale_order_line_model.create({"product_id": self.product_1.id, "product_uom_qty": 8, "order_id": so.id})
        self.sale_order_line_model.create({"product_id": self.product_2.id, "product_uom_qty": 8, "order_id": so.id})
        # confirm quotation
        so.action_confirm()
        self.assertEqual(
            len(so.picking_ids),
